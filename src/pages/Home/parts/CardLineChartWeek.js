...
export default function CardLineChartWeek({ time, consumption, flow }) {
  console.log(time);
  ...
  React.useEffect(() => {
    ...
    let ctx = document.getElementById("weekchartLineHome").getContext("2d");
    window.chartWeek && window.chartWeek.destroy();
    window.chartWeek = new Chart(ctx, config);
      ...
      <div className="relative flex flex-col min-w-0 break-words bg-blueGray-700 w-full">
        ...
        <div className="px-4 flex-auto">
          <div className="relative h-screen">
            <canvas id="weekchartLineHome"></canvas>
          </div>
        </div>
      </div>
    </>
